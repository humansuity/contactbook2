...
class ContackBookAppTestUpdateDBAppium(unittest.TestCase):
    ...
    def test_enter(self):
        ...
        #1. Переход во вкладку ГПО "Белтопгаз"
        ...
        #1.1. Поиск элемента ГПО "Белтопгаз"
        ...
        for element in elementsDepartament:
            if element.text == "Управление газообеспечения и маркетинга":
                element.click()
                time.sleep(2)
                break
        ...
        #3.1. Делаем поиск по должности. В данном тесте должность "Ведущий инженер".
        elementTextPost = self.driver.find_elements_by_id("net.gas.gascontact:id/text_post")

        for element in elementTextPost:
            if element.text == "Ведущий инженер":
                element.click()
                time.sleep(2)
                break

        #4. Поиск элементов: Мобильный телефон
